...
class PhysicsPriorLoss(nn.Module):
    ...
    def _fog_loss(self, info, fake_image, pseudo_depth):
        ...
        雾天物理先验损失（修正版）
        """
        depth = info['depth']  # (B,1,256,256)
        A = info['A']  # (B,3,1,1)

        # === 归一化伪深度标签到 [0,1] ===
        # 防止除零，加小常数
        pseudo_depth_norm = pseudo_depth / (pseudo_depth.max(dim=2, keepdim=True)[0].max(dim=3, keepdim=True)[0] + 1e-8)

        # === 1. 深度辅助损失 ===
        loss_depth = self.l1_loss(depth, pseudo_depth_norm)

        # === 2. 暗通道约束损失 ===
        dark = dark_channel(fake_image, patch_size=15)
        A_lum = A.mean(dim=1, keepdim=True)
        A_lum_expanded = A_lum.expand_as(dark)
