...
def main():
    ...
    if args.auto_schedule:
        ...
        schedule = {
            "stages": [
                # 阶段1 (epoch 0-30)：主攻感知+语义，轻量对抗
                {"end_epoch": 30, "adv": 0.5, "perc": 1.0, "sem": 1.0, "phys": 0.3},
                # 阶段2 (epoch 30-100)：均衡发展
                {"end_epoch": 100, "adv": 0.5, "perc": 1.0, "sem": 1.0, "phys": 0.3},
                # 阶段3 (epoch 100-200)：强化对抗+物理细节
                {"end_epoch": 200, "adv": 0.5, "perc": 1.0, "sem": 1.0, "phys": 0.3},
            ]
        }
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for (clear_img, clear_sem), (adv_img, adv_sem) in pbar:
            ...
            loss_adv = loss_dict_G['loss_adv']
            loss_sem = loss_dict_G['loss_sem']
            loss_phys = loss_dict_G['loss_phys']
            loss_perc = loss_dict_G['loss_perc']
