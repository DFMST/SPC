...
from utils import (
    ...
    visualize_physics, visualize_complete_workflow
)
...
def parse_args():
    ...
    parser.add_argument("--resume", type=str, default="output/checkpoints/sem_cyclegan_epoch_50.pth", help="Path to checkpoint to resume from")
    ...
    parser.add_argument("--physics_sample_interval", type=int, default=200,
    ...
    parser.add_argument("--weather_type", type=str, default="fog",
...
def main():
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for (real_A, sem_A), (real_B, sem_B) in pbar:
            ...
            if global_step % args.sample_interval == 0:
                with torch.no_grad():
                    ...
                    visualize_batch(
                        ...
                        sem_A, outputs['sem_hat_A'],
                        save_path=save_path,
                        title=f"训练样本 - Step {global_step}",
                        step=global_step
                    )
            ...
            # # 定期保存物理可视化
            # if global_step % args.physics_sample_interval == 0:
            #     with torch.no_grad():
            #         save_path = os.path.join(args.output_dir, "physics_samples", f"physics_{global_step}")
            #         visualize_physics(model, real_A, outputs['fake_B'], outputs['fake_B_dict'], save_path)
            #         writer.add_image('Physics/Visualization',
            #                          plt.imread(f"{save_path}_physics.png").transpose(2, 0, 1),
            #                          global_step)
        ...
        #
        #     # 获取一批验证数据（多个样本）
        #     val_batch = next(iter(val_loader_A))
        #     real_A_val = val_batch[0].to(args.device)
        #     sem_A_val = val_batch[1].to(args.device)
        #
        #     # 限制样本数量，避免内存问题
        #     num_viz_samples = min(4, real_A_val.size(0))
        #     real_A_val = real_A_val[:num_viz_samples]
        #     sem_A_val = sem_A_val[:num_viz_samples]
        #
        #     with torch.no_grad():
        #         # 前向传播获取所有输出
        #         outputs = model(real_A_val, real_A_val)  # 使用相同数据作为B域
        #
        #         # 调试：打印fake_B_dict中包含的键
        #         print("fake_B_dict keys:", list(outputs['fake_B_dict'].keys()))
        #         print("fake_B_dict shapes:")
        #         for key, value in outputs['fake_B_dict'].items():
        ...
        #             real_A=real_A_val,
        #             sem_A=sem_A_val,
        #             fake_B_dict=outputs['fake_B_dict'],
        #             rec_A=outputs['rec_A'],
        #             epoch=epoch,
        #             global_step=global_step,
        #             save_dir='output/10.31',
        #             device=args.device,
