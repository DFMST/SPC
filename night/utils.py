...
class SemanticDrivingDataset(Dataset):
    def __init__(self, root_dir, domain='A', transform=None, phase='train',
        ...
        # 空间变换
        # ---------- 图像的空间变换（CenterCrop + Resize，不含归一化） ----------
        ...
        # 如果外部未提供 transform，使用默认的 ToTensor + Normalize
    ...
    def __getitem__(self, idx):
        ...
        # 正确读取语义图索引
        if sem.mode == 'P':
            sem_arr = np.array(sem)  # (H, W) uint8
        elif sem.mode == 'RGB':
            raise RuntimeError(f"语义图是 RGB 模式，请确认文件类型: {semantic_path}")
        else:
            sem_arr = np.array(sem.convert('L'))

        # 映射到自定义类别（0~5）
        sem_arr = map_acdc_to_custom(sem_arr)

        # 图像的空间变换（裁剪缩放）
        image = self.spatial_transform(image)  # 仍是 PIL Image

        # 语义图的空间变换（NEAREST）
        sem_pil = Image.fromarray(sem_arr.astype(np.uint8))
        sem_resized = self.sem_transform(sem_pil)
        semantic = torch.from_numpy(np.array(sem_resized)).long()  # (H, W) LongTensor

        # 图像归一化（ToTensor + Normalize）
        image = self.transform(image)  # 确保变为 tensor
...
def visualize_batch(real_A, fake_B, real_B, semantic_A, semantic_hat,
                    save_path=None, title="Training Samples", step=0):
    """
    可视化训练过程中的批量样本（单向生成，无重建）。

    参数：
        real_A: 清晰域图像 (B,3,H,W)
        fake_B: 生成的恶劣天气图像 (B,3,H,W)
        real_B: 真实的恶劣天气图像 (B,3,H,W)
        semantic_A: 清晰域的语义标签 (B,H,W) 或 (B,1,H,W)
        semantic_hat: 预测的语义图 (B,C,H,W) logits 或 (B,H,W) 索引图
    """
    ...
    img_real_B = safe_denorm(real_B[0]).cpu().permute(1, 2, 0).numpy()

    # ---- 语义差异图 ----
    # 1. 真实标签
    sem_A = semantic_A[0].cpu()
    if sem_A.dim() == 3 and sem_A.shape[0] == 1:
        sem_A = sem_A.squeeze(0)
    sem_A_np = sem_A.numpy()

    # 2. 预测类别图
    if semantic_hat.dim() == 4:
        pred_class = semantic_hat.argmax(dim=1)
    elif semantic_hat.dim() == 3:
        pred_class = semantic_hat
    else:
        raise ValueError(f"Unexpected semantic_hat dimension: {semantic_hat.dim()}")

    sem_hat = pred_class[0].cpu()
    if sem_hat.dim() == 1:
        sem_hat = sem_hat.unsqueeze(0)
    sem_hat_np = sem_hat.numpy()

    # 3. 尺寸对齐
    if sem_hat_np.shape != sem_A_np.shape:
        sem_hat_tensor = torch.from_numpy(sem_hat_np).unsqueeze(0).unsqueeze(0).float()
        sem_hat_resized = F.interpolate(sem_hat_tensor, size=sem_A_np.shape, mode='nearest')
        sem_hat_np = sem_hat_resized.squeeze(0).squeeze(0).long().numpy()

    diff = np.abs(sem_A_np - sem_hat_np)
    ...
    axes[2].set_title("Real B")  # 原来叫 "Reconstructed A"，现在更正
    axes[2].axis('off')
...
def save_checkpoint(model, optimizer_G, optimizer_D, epoch, save_dir, filename_prefix='spc'):
    """保存模型检查点（适配新模型 SPCGenerator + MultiScaleDiscriminator）"""
...
def load_checkpoint(model, optimizer_G, optimizer_D, checkpoint_path, device='cpu'):
    ...
    # 加载模型状态
    model.generator.load_state_dict(state['generator_state'])
    model.discriminator.load_state_dict(state['discriminator_state'])
...
def compute_semantic_metrics(model, dataloader, device, n_classes=6):
    ...
    # 获取语义解码器（同现有代码）
    try:
        sem_decoder = model.loss_calculator.sem_loss.sem_decoder
    except AttributeError:
        sem_decoder = nn.Sequential(
    ...
    with torch.no_grad():
        for images, sem_targets in tqdm(dataloader, desc="Computing metrics"):
            ...
            # 获取语义特征
            sem_features, _ = model.generator.semantic_encoder(images)
            # 解码语义图
            sem_pred = sem_decoder(sem_features)  # (B, n_classes, H', W')
            ...
            # ========== 新增：将目标标签下采样到预测图尺寸 ==========
            if target_classes.shape[-2:] != pred_classes.shape[-2:]:
                # 使用最近邻插值保持离散标签值
                target_classes = F.interpolate(
                    target_classes.float().unsqueeze(1),   # (B,1,H,W)
                    size=pred_classes.shape[-2:],          # (H', W')
                    mode='nearest'
