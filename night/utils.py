...
def visualize_batch(real_A, fake_B, rec_A, semantic_A, semantic_hat,
    ...
    # 确保数据类型正确
    img_A = np.clip(img_A, 0, 1).astype(np.float32)
    img_fake_B = np.clip(img_fake_B, 0, 1).astype(np.float32)
    img_rec_A = np.clip(img_rec_A, 0, 1).astype(np.float32)
...
def safe_denorm(tensor):
    """将[-1,1]范围的张量转换为[0,1]范围的图像，确保正确的数据类型"""
    denormed = tensor.clone()
    for c in range(3):
        denormed[c] = denormed[c] * 0.5 + 0.5
    # 确保数据类型
    denormed = denormed.clamp(0, 1)
    return denormed
...
def debug_semantic_categories(model, dataloader, device, num_samples=3):
    """简单直接地显示语义类别对应关系"""
    model.eval()
    ...
    with torch.no_grad():
        for i, (images, sem_targets) in enumerate(dataloader):
            if i >= num_samples:
                break
            ...
            # 转换为类别索引
            pred_classes = sem_pred.argmax(1)

            # 打印类别统计
            print(f"\n样本 {i + 1}:")
            print("预测类别分布:")
            for class_id in range(6):
                count = (pred_classes == class_id).sum().item()
                percentage = count / pred_classes.numel() * 100
                print(f"  类别 {class_id}: {count} 像素 ({percentage:.1f}%)")

            # 显示原始图像和语义图
            img = images[0].cpu()
            img = (img * 0.5 + 0.5).clamp(0, 1).permute(1, 2, 0).numpy()

            sem_vis = pred_classes[0].cpu().numpy()
            ...
            plt.subplot(1, 2, 2)
            # 修复：使用只包含6种颜色的自定义颜色映射
            from matplotlib.colors import ListedColormap
            colors = ['black', 'red', 'green', 'blue', 'yellow', 'purple']
            cmap = ListedColormap(colors)

            plt.imshow(sem_vis, cmap=cmap, vmin=0, vmax=5)
            plt.title("语义分割 (颜色对应类别索引)")

            # 修复：只显示6个类别的颜色条
            cbar = plt.colorbar(ticks=[0, 1, 2, 3, 4, 5], label='类别索引')
            cbar.set_ticklabels(['0', '1', '2', '3', '4', '5'])
            plt.axis('off')

            plt.tight_layout()
            plt.savefig(f'semantic_debug_{i}.png', dpi=120, bbox_inches='tight')
            plt.close()

            print(f"语义可视化已保存: semantic_debug_{i}.png")
