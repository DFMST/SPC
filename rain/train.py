...
def main():
    ...
    if args.auto_schedule:
        ...
        schedule = {
            "stages": [
                {"end_epoch": 20, "adv": 1.0, "perc": 1.0, "sem": 1.0, "phys": 0.1},
                {"end_epoch": 80, "adv": 1.0, "perc": 1.0, "sem": 1.0, "phys": 0.1},
                {"end_epoch": 140, "adv": 1.0, "perc": 1.0, "sem": 1.0, "phys": 0.1},
                {"end_epoch": 200, "adv": 1.0, "perc": 1.0, "sem": 1.0, "phys": 0.1},
            ]
        }
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for (clear_img, clear_sem), (adv_img, adv_sem) in pbar:
            ...
            loss_G.backward()
            # 梯度裁剪
            torch.nn.utils.clip_grad_norm_(model.generator.parameters(), max_norm=10.0)
            optimizer_G.step()
