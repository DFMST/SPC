...
from utils import (
    ...
    visualize_physics, save_individual_components, visualize_physics_separate
)
...
def parse_args():
    ...
    parser.add_argument("--physics_sample_interval", type=int, default=10,
...
def main():
    ...
    sample_A, sem_A = next(iter(train_loader_A))
    sample_B, sem_B = next(iter(train_loader_B))

    print(f"A域样本形状: {sample_A.shape}, 语义图形状: {sem_A.shape}")
    print(f"B域样本形状: {sample_B.shape}, 语义图形状: {sem_B.shape}")

    # 检查语义图中的唯一值
    print(f"A域语义图唯一值: {torch.unique(sem_A)}")
    print(f"B域语义图唯一值: {torch.unique(sem_B)}")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for (real_A, sem_A), (real_B, sem_B) in pbar:
            ...
            #     with torch.no_grad():
            #         save_dir = os.path.join(args.output_dir, "physics_samples", f"step_{global_step}")
            #         visualize_physics_separate(
            #             model,
            #             real_A,
            #             outputs['fake_B'],
            #             sem_A,  # 语义分割图
