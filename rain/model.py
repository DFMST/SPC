...
class RaindropSimulator(nn.Module):
    def __init__(self):
        ...
        # 增强风场效果
        self.register_buffer('wind_field', torch.tensor([0.8, 0.8]))
    ...
    def forward(self, x, intensity):
        ...
        # 增大雨滴参数范围
        drop_size = torch.sigmoid(params[:, 0:1]) * 3.0 + 1.0  # 1-4像素
        density = torch.sigmoid(params[:, 1:2]) * 0.8 * intensity  # 密度系数增加到0.8

        # 增强噪声效果
        noise = torch.randn_like(x) * density * 1.2

        # 添加更多高频细节
        high_freq_noise = torch.randn(x.size(0), 1, x.size(2) * 4, x.size(3) * 4, device=x.device)
        high_freq_noise = F.interpolate(high_freq_noise, size=x.shape[2:], mode='bilinear')
        high_freq_noise = high_freq_noise.repeat(1, 3, 1, 1) * density * 0.8

        combined_noise = (noise + high_freq_noise) / 1.5

        # 增强运动模糊 - 修复卷积核维度
        avg_drop_size = drop_size.mean(dim=[2, 3], keepdim=True)
        motion_kernel = self._create_strong_motion_kernel(avg_drop_size)

        # 修复：为每个通道创建独立的卷积核
        rain_effect = self._apply_motion_blur(combined_noise, motion_kernel)

        # 大幅增强整体效果
        rain_effect = rain_effect * intensity * 1.2

        return torch.clamp(rain_effect, -0.3, 0.3)

    def _create_strong_motion_kernel(self, drop_size):
        """创建更明显的运动模糊核"""
        kernel_size = min(15, max(7, int(drop_size.mean().item() * 3.0)))
        ...
        for i in range(kernel_size):
            for j in range(kernel_size):
                ...
                # 使用更宽的高斯分布，使模糊效果更明显
                kernel[0, 0, i, j] = math.exp(-(dist ** 2) / (0.8 * center ** 2))

        return kernel / (kernel.sum() + 1e-6)

    def _apply_motion_blur(self, x, kernel):
        """应用运动模糊 - 修复版本"""
        # 为每个通道应用相同的卷积核
        b, c, h, w = x.shape
        ...
        channels = []
        for i in range(c):
            channel_input = x[:, i:i + 1, :, :]  # [B, 1, H, W]
            # 应用卷积，padding='same'保持尺寸
            blurred_channel = F.conv2d(
                channel_input,
                kernel,
                padding=kernel.shape[-1] // 2,  # 计算合适的padding
                groups=1
            )
...
class PhysicsInformedRainLayer(nn.Module):
    ...
    def __init__(self, input_dim=128):
        ...
        self.weather_param_predictor = nn.Sequential(
            ...
            nn.Flatten(),
            nn.Linear(input_dim, 16),  # 减小网络大小
            nn.ReLU(),
            nn.Linear(16, 1),  # 只输出雨强度
    ...
    def forward(self, base_scene, semantic_features):
        # 预测雨强度，并限制最大强度
        rain_intensity = torch.sigmoid(self.weather_param_predictor(semantic_features)) * 0.6  # 限制最大为0.6

        # 生成更细微的雨滴效果
        rain_effect = self.raindrop_simulator(base_scene, rain_intensity)

        # 进一步减弱效果，确保雨滴不明显
        rain_effect = rain_effect * 0.8
...
class SnowSimulator(nn.Module):
    ...
    def forward(self, x, intensity):
        ...
        # 限制雪片大小和密度范围
        flake_size = 2 + 4 * torch.sigmoid(params[:, 0:1])  # 2-6像素
        density = 0.2 + 0.5 * torch.sigmoid(params[:, 1:2])  # 限制密度范围0.2-0.7
        density = density * intensity  # 受强度控制
        ...
        # 关键优化：限制雪效强度和覆盖范围
        snow_effect = snow_effect * intensity * 0.6  # 降低效果强度
        return torch.clamp(snow_effect, 0, 0.8)  # 确保不覆盖超过80%的场景
...
class EnhancedSceneParser(nn.Module):
    ...
    def __init__(self, semantic_dim):
        ...
        self.region_classifier = nn.Sequential(
            ...
            nn.ReLU(),
            # 移除可能问题的EdgeAwareBlock
            nn.Conv2d(128, 64, 3, padding=1),
            nn.ReLU(),
            nn.Conv2d(64, 3, 1),
            nn.Sigmoid()  # 改为Sigmoid激活
        )
        nn.init.xavier_uniform_(self.region_classifier[-2].weight)
        nn.init.constant_(self.region_classifier[-2].bias, 0.1)
...
class RegionAwareFusion(nn.Module):
    ...
    def forward(self, base_scene, weather_effect, region_mask, weather_type=None):
        """
        新增weather_type参数，用于区分天气类型
        """
        # 建筑边缘抑制（通用逻辑）
        building_edges = self.building_edge_detector(base_scene)
        suppressed_edges = self.edge_suppressor(base_scene)
        ...
        if weather_type == "rain":
            ...
            # 不同区域不同的雨滴强度
            sky_effect = weather_effect * sky_mask * 1.2  # 天空区域最强
            dynamic_effect = weather_effect * dynamic_mask * 0.9  # 动态物体区域中等
            ground_effect = weather_effect * ground_mask * 0.7  # 地面区域较弱

            # 添加全局雨滴效果（减弱版）
            global_effect = weather_effect * 0.3

            return base_scene + sky_effect + dynamic_effect + ground_effect + global_effect
        else:
            # 其他天气保持原有逻辑
...
class PhysicsInformedSnowLayer(nn.Module):
    ...
    def forward(self, base_scene, semantic_features):
        ...
        # 远景区域减少雪效
        snow_effect = snow_effect * (1 - depth_mask * 0.6)
...
class SemanticCycleGAN(nn.Module):
    ...
    def compute_generator_loss(self, real_A, real_B, sem_A, sem_B, outputs, epoch, total_epochs):
        ...
        # 总损失
        if epoch < total_epochs * 0.3:  # 前30%epoch保持较强约束
            w_physics = 1.0
        elif epoch < total_epochs * 0.6:  # 中间30%epoch中等约束
            w_physics = 0.5
        else:  # 后40%epoch放宽约束
            w_physics = 0.2

            # 增加对抗损失权重，鼓励更明显的效果
        w_adv = 1.5  # 从1.0增加到1.5

        loss_G = (w_adv * loss_adv + w_cycle * loss_cycle +
                  w_sem * loss_sem + w_physics * physics_loss)
    ...
    def compute_physics_loss(self, fake_dict, fake_img):
        weather_type = fake_dict.get('weather_type', 'rain')
        base_loss = torch.tensor(0.0, device=fake_img.device)

        if 'weather_effect' in fake_dict and 'region_mask' in fake_dict:
            ...
            if weather_type == "rain":
                ...
                sky_rain = (weather_effect * sky_mask).mean()
                non_sky_rain = (weather_effect * non_sky_mask).mean()

                # 希望天空区域的雨滴稍强，但不是严格限制
                if sky_rain > 1e-6:  # 避免除零
                    ratio = non_sky_rain / (sky_rain + 1e-6)
                    # 如果非天空区域雨滴太强，给予轻微惩罚
                    if ratio > 0.8:  # 非天空区域雨滴强度不超过天空区域的80%
                        base_loss += (ratio - 0.8) * 0.1

                # 鼓励适当的雨滴密度
                mean_effect = weather_effect.mean()
                if mean_effect < 0.05:  # 如果雨滴效果太弱
                    base_loss += (0.05 - mean_effect) * 0.5
