...
def load_model(model_path, device, weather_type="snow"):
    """
    加载训练好的 SPCModel 生成器
    """
    # 创建模型实例（仅生成器部分会被加载）
...
def main():
    ...
    parser.add_argument("--model_path", type=str,
                        default="./output/checkpoints/spc_epoch_20.pth",
                        help="训练好的模型路径(.pth文件)")
    parser.add_argument("--data_dir", type=str, default="../eva_data/ALL",
                        help="输入图像目录")
    parser.add_argument("--output_dir", type=str, default="../output_data/ALL/snow",
                        help="输出目录")
