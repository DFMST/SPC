...
class UpBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, scale_factor=2):
        ...
        # 使用插值上采样+卷积，避免PixelShuffle的网格效应
...
class PhysicsInformedSnowLayer(nn.Module):
    """重构的物理约束雪天生成层-支持局部温度调控"""

    def __init__(self, input_dim=128):
        super().__init__()

        # 温度场预测网络 - 预测空间变化的温度分布
        self.temperature_field_predictor = nn.Sequential(
            nn.Conv2d(3, 64, 3, padding=1),  # 输入基础场景
            nn.LeakyReLU(0.2),
        ...
        # 温度调控参数（可学习）
        self.temperature_effect_scale = nn.Parameter(torch.tensor(2.0))  # 温度影响尺度
        self.snow_melt_threshold = nn.Parameter(torch.tensor(0.7))  # 融化阈值

    def forward(self, base_scene, semantic_features):
        B, C, H, W = base_scene.shape

        # 预测全局雪强度
        snow_intensity = torch.sigmoid(self.weather_param_predictor(semantic_features))

        # 预测空间变化的温度场
        temperature_field = self.temperature_field_predictor(base_scene)  # [B, 1, H, W]

        # 预测全局场景温度（用于整体调节）
        global_temperature = self.temp_predictor(semantic_features)
        ...
        # 温度局部调控机制
        with torch.no_grad():
            # 1. 控制雪花生成概率 - 温度高的区域雪花减少
            snow_gen_prob = 1.0 - temperature_field * 0.8  # 温度越高，雪花生成概率越低
            snow_effect = snow_effect * snow_gen_prob

            # 2. 调节积雪积累速率 - 温度低的区域积累更快
            accumulation_rate = 1.0 + (1.0 - temperature_field) * self.temperature_effect_scale
            snow_effect = snow_effect * accumulation_rate

            # 3. 模拟融化现象 - 温度超过阈值时发生融化
            melt_mask = (temperature_field > self.snow_melt_threshold).float()
            melt_factor = 1.0 - (temperature_field - self.snow_melt_threshold) * 3.0  # 融化强度
            melt_factor = torch.clamp(melt_factor, 0.1, 1.0)
            snow_effect = snow_effect * (1.0 - melt_mask * (1.0 - melt_factor))

        # 全局温度控制（保持原有逻辑）
        temp_factor = torch.where(
            global_temperature < 0.3,
            1.0 + (0.3 - global_temperature) * 2,
            torch.clamp(1.0 - (global_temperature - 0.7) * 3, 0.2, 1.0)
        )
        snow_effect = snow_effect * temp_factor.view(-1, 1, 1, 1)

        # 深度感知约束
        with torch.no_grad():
            depth = self.depth_predictor(base_scene)
            depth_mask = (depth > 0.7).float()
            # 在深度大的区域（如天空），根据温度调整雪效
            sky_snow_reduction = temperature_field * 0.6  # 温度高的天空区域雪效更弱
            snow_effect = snow_effect * (1 - depth_mask * sky_snow_reduction)

        return snow_effect, temperature_field, snow_intensity
...
class SnowSimulator(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        self.snow_learner = nn.Sequential(
            nn.Conv2d(3, 32, 3, padding=1),
        ...
        self.temperature_adapter = nn.Sequential(
            nn.Conv2d(1, 16, 3, padding=1),  # 输入温度场
            nn.ReLU(),
            nn.Conv2d(16, 3, 1),  # 输出雪效调整参数
            nn.Tanh()
        )
    ...
    def forward(self, x, intensity):
        ...
        # 基础雪效学习
        snow_adjustment = self.snow_learner(x) * 0.2

        # 如果有温度场输入，进行温度适配
        if hasattr(self, 'temperature_field'):
            temp_adjustment = self.temperature_adapter(self.temperature_field)
            snow_adjustment = snow_adjustment * (1 + temp_adjustment * 0.5)

        return snow_adjustment * intensity
...
class NightAdjuster(nn.Module):
    def __init__(self):
        ...
        # 天空光源抑制器
        self.sky_suppressor = nn.Sequential(
            nn.Conv2d(1, 16, 3, padding=1),  # 输入深度图
            nn.ReLU(),
            nn.Conv2d(16, 1, 1),
            nn.Sigmoid()
        )
    ...
    def forward(self, x, intensity):
        ...
        # 光照预测（限制最大亮度）
        L_base = self.light_predictor(x)
        L = L_base * 0.6  # 最大亮度限制在0.6

        # 识别天空区域（深度>0.7）
        sky_mask = (depth > 0.7).float()

        # 抑制天空区域的光照强度
        sky_suppression = self.sky_suppressor(depth)
        L = L * (1 - sky_mask * sky_suppression * 0.9)  # 减少90%的天空光照

        # 物理光照模型（原版公式）
        beta = 0.1 + intensity * 2.0
        transmission = torch.exp(-beta * depth)

        # 最终合成
        night_effect = x * transmission + L * (1 - transmission)
...
class RegionAwareFusion(nn.Module):
    def __init__(self):
        ...
        # 边缘白光抑制器
        self.edge_suppresor = nn.Sequential(
            nn.Conv2d(3, 16, 3, padding=1),
            nn.ReLU(),
            nn.Conv2d(16, 3, 1),
            nn.Sigmoid()
        ...
        # 雪天专用模块
        self.snow_depth_predictor = nn.Sequential(
        ...
        self.snow_accumulation_enhancer = nn.Sequential(
            nn.Conv2d(3, 64, 3, padding=1),
            nn.ReLU(),
            nn.Conv2d(64, 32, 3, padding=1),
            nn.ReLU(),
        ...
        # 雪效增强平滑层
        self.snow_enhance_smooth = nn.Conv2d(3, 3, 3, padding=1, groups=3, bias=False)
        nn.init.constant_(self.snow_enhance_smooth.weight, 1 / 9)  # 平均平滑

        # 温度感知的融合门控
        self.temperature_fusion_gate = nn.Sequential(
            nn.Conv2d(4, 16, 3, padding=1),  # 输入: RGB + 温度场
            nn.ReLU(),
            nn.Conv2d(16, 3, 1),
            nn.Sigmoid()
        )
    ...
    def forward(self, base_scene, weather_effect, region_mask, weather_type=None):
        ...
        if weather_type == "snow":
            ...
            # 减弱雪效强度，更自然融合
            weather_effect = weather_effect * 0.3

            # 主要在地面区域应用雪效
            ground_snow_effect = weather_effect * ground_mask * 0.8

            # 轻微的天空区域效果(阴天感)
            sky_mask = region_mask[:, 0:1]
            sky_snow_effect = weather_effect * sky_mask * 0.1

            # 组合效果
            final_effect = ground_snow_effect + sky_snow_effect

            # 与基础场景自然融合
            return base_scene * (1 - 0.2) + final_effect + base_scene * 0.2
...
class DisentangledGenerator(nn.Module):
    ...
    def forward(self, x, semantic_features):
        ...
        # 场景解析
        region_mask = self.scene_parser(semantic_features)  # [B, 3, 32, 32]

        # 物理天气生成
        if self.weather_type == "snow":
            weather_effect, temperature_field, rain_intensity = self.weather_layer(base_scene, semantic_features)
            fog_intensity = torch.zeros_like(rain_intensity)
        else:
            weather_effect, rain_intensity, fog_intensity = self.weather_layer(base_scene, semantic_features)
            temperature_field = None

        # 上采样区域掩码匹配天气效果分辨率
        region_mask = self.upsample(region_mask)  # [B, 3, 256, 256]

        # 区域感知融合-添加 weather_type 参数
        output = self.fusion_module(
            base_scene,
            weather_effect,
            region_mask,
            weather_type=self.weather_type  # 新增天气类型参数
        )
        ...
        return {
            ...
            "temperature_field": temperature_field,  # 新增输出
...
class PhysicsInformedSnowLayer(nn.Module):
    ...
    def forward(self, base_scene, semantic_features):
        ...
        # 预测场景温度
        temperature = self.temp_predictor(semantic_features)
        ...
        temp_factor = torch.where(
            temperature < 0.3,
            1.0 + (0.3 - temperature) * 2,
            torch.clamp(1.0 - (temperature - 0.7) * 3, 0.2, 1.0)
        )
        snow_effect = snow_effect * temp_factor.view(-1, 1, 1, 1)
        ...
        with torch.no_grad():
            depth = self.depth_predictor(base_scene)
            depth_mask = (depth > 0.7).float()
            snow_effect = snow_effect * (1 - depth_mask * 0.4)
...
class SemanticCycleGAN(nn.Module):
    ...
    def compute_generator_loss(self, real_A, real_B, sem_A, sem_B, outputs, epoch, total_epochs):
        ...
        physics_loss = torch.clamp(physics_loss, min=-5.0, max=5.0)  # 限制范围


        # 动态损失权重
        w_physics = 0.2  # 限制最大权重
        # 动态损失权重(随训练进度调整)
        w_adv = 1.0
        w_cycle = 8
        w_sem = 1.5  # 语义损失随训练衰减
    ...
    def compute_physics_loss(self, fake_dict, fake_img):
        weather_type = fake_dict['weather_type']
        ...
        if weather_type == "snow":
            ...
            # 放宽雪效约束，让模型更自由学习
            # 1. 覆盖范围约束更宽松
            coverage = (torch.abs(weather_effect) > 0.01).float().mean()
            base_loss += F.relu(coverage - 0.4) * 1.0  # 最高40%覆盖
            base_loss += F.relu(0.05 - coverage) * 1.0  # 至少5%覆盖

            # 2. 效果强度约束（避免过强）
            effect_strength = torch.mean(torch.abs(weather_effect))
            base_loss += F.relu(effect_strength - 0.2) * 2.0  # 强度不超过0.2

            # 3. 自然度约束（鼓励平滑过渡）
            # 修改拉普拉斯计算以处理多通道输入
            # 首先将天气效果转换为灰度图
            gray_coeffs = torch.tensor([0.2989, 0.5870, 0.1140],
                                       device=weather_effect.device).view(1, 3, 1, 1)
            weather_gray = torch.sum(weather_effect * gray_coeffs, dim=1, keepdim=True)
            ...
            # 应用拉普拉斯算子到灰度图
            laplacian = F.conv2d(weather_gray, laplacian_kernel, padding=1)
            smoothness = torch.mean(torch.abs(laplacian))
            base_loss += smoothness * 0.1  # 鼓励平滑
