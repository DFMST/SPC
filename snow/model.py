...
class BaseSceneGenerator(nn.Module):
    ...
    输出: I_scene (B,3,256,256) — 保持输入结构的残差输出
    """
    def __init__(self):
        super().__init__()
        ...
        self.out_layer = nn.Sequential(
            ...
            # 注意：此处不使用 Tanh，因为残差将在外部与输入相加后统一 Tanh
        )
    ...
    def forward(self, I_in, F_sem):
        ...
        w = weather_vec.expand(-1, -1, 64, 64)
        x = torch.cat([x, w], dim=1)
        x = self.cond_inject_up1(x)

        x = self.up2(x)                            # (B,128,128,128)
        x = x + skip_down1
        w = weather_vec.expand(-1, -1, 128, 128)
        x = torch.cat([x, w], dim=1)
        x = self.cond_inject_up2(x)

        x = self.up3(x)                            # (B,64,256,256)
        x = x + skip_init
        w = weather_vec.expand(-1, -1, 256, 256)
        x = torch.cat([x, w], dim=1)
        x = self.cond_inject_up3(x)

        # ---- 5. 残差输出 ----
        residual = self.out_layer(x)               # (B,3,256,256) 未激活
        I_scene = torch.tanh(I_in + residual)      # 残差学习，保持输入结构
        return I_scene
...
class LearnableGaussianBlur(nn.Module):
    """
    可学习标准差的高斯模糊层
    sigma 通过 Softplus 确保为正，由网络预测得到
    """
    ...
    def _create_gaussian_kernel(self, sigma, kernel_size=None):
        ...
        if kernel_size is None:
            # 根据 sigma 自动确定核大小（至少 3，最大 max_kernel_size，奇数）
            k = int(2 * torch.ceil(3 * sigma).max().item() + 1)
        ...
        x = coords.view(1, -1).repeat(kernel_size, 1)  # (K, K)
        y = coords.view(-1, 1).repeat(1, kernel_size)
        # 对于每个样本，计算高斯核
        # sigma: (B,1,1,1) 方便广播
        sigma = sigma.view(-1, 1, 1, 1)  # (B,1,1,1)
        g = torch.exp(-(x**2 + y**2) / (2 * sigma**2))  # (B, K, K)
        g = g / g.sum(dim=(1,2), keepdim=True)  # 归一化
        g = g.unsqueeze(1)  # (B,1,K,K)
        return g
    ...
    def forward(self, x, sigma):
        ...
        B, C, H, W = x.shape
        # 生成高斯核
        kernel = self._create_gaussian_kernel(sigma)  # (B,1,K,K)
        K = kernel.shape[-1]
        padding = K // 2
        # 对每个通道分别卷积（groups=C）
        # 需要将 kernel 扩展到每个通道: (B,1,K,K) -> (B*C,1,K,K) 然后使用 groups=B*C?
        # 更好的方式：对每个样本每个通道单独卷积，使用 group 参数
        # 将 x 变为 (B*C, 1, H, W)，kernel 变为 (B*C, 1, K, K)，groups=B*C
        x_group = x.view(1, B*C, H, W)  # (1, B*C, H, W)
        kernel_group = kernel.repeat_interleave(C, dim=0)  # (B*C, 1, K, K)
        out = F.conv2d(x_group, kernel_group, padding=padding, groups=B*C)
...
class MultiScaleDiscriminator(nn.Module):
    ...
    def _make_scale(self):
        return nn.Sequential(
            nn.utils.spectral_norm(nn.Conv2d(3, 64, 4, 2, 1, bias=True)),
            ...
            nn.InstanceNorm2d(256),
            nn.LeakyReLU(0.2, True),
            nn.utils.spectral_norm(nn.Conv2d(256, 1, 4, 1, 1, bias=True))
        )
